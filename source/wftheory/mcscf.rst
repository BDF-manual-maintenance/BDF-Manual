...
* 若是采用Superci还未收敛，也可以将优化的轨道作为初始猜测，然后改为Quasi提高收敛性。

.. attention::
  此方法的近似较大，可能导致MCSCF轨道优化收敛拖尾的现象。若出现此情况，可通过增大关键词Conv的阈值。

:guilabel:`SwitchIter` 参数类型：整型
------------------------------------------------
指定开始使用DIIS加速Super-CI-PT方法优化分子轨道时的宏迭代步数。默认值：15

:guilabel:`SwitchConv` 参数类型：浮点型
------------------------------------------------
指定开始使用DIIS加速Super-CI-PT方法优化分子轨道时的梯度阈值。默认值：0.01

:guilabel:`NmoCri` 参数类型：浮点型
------------------------------------------------
用于控制活性轨道自然轨道占据数接近双占据轨道（Close）与未占据轨道（Virtual）的情况。
* 默认值：2.0 0.0 表示未控制。
* 建议值：1.999 0.001 表示占据数大于1.999的轨道不与close轨道交换，而小于0.001的轨道不与Virtual轨道交换。
