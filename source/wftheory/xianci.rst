...
:guilabel:`Maxbloch` 参数类型：整型
------------------------------------------------
指定迭代求解CASPT2、SDSPT2f、SDSCIf所需的BLOCH方程的最大迭代次数。
* 默认值：5
...
:guilabel:`DoProp` 参数类型：Bool型
------------------------------------------------
指定计算单电子(跃迁)约化密度矩阵及相关的性质，如（跃迁）偶极矩等。
* 打印自然轨道占据数。
* 提供自然轨道文件$BDF_WORKDIR/$BDFTASK.xianciorb。
* 提供轨道图像文件$BDF_WORKDIR/$BDFTASK.xianci.molden。
