...
作业完成后在log文件中找到最后一个Excited State 1为 :math:`S_{1}`激发能，Total Energy为电子态能量。
...
激发态 :math:`T_{2}` 、 :math:`T_{3}` 优化
-------------------------------------------

由于本案例后续会用MOMAP做 :math:`T_{2}` → :math:`S_{1}` 态和 :math:`T_{3}` → :math:`S_{1}` 态的反系间窜越速率计算，所以前期还需要量化软件做激发态 :math:`T_{2}` 和 :math:`T_{3}` 的结构优化和频率计算，得到log文件和fchk文件。泛函和基组分别为M062x和Def2SVP，T2.gjf文件如下：
...
作业完成后在log文件中找到最后一个Excited State 2为 :math:`T_{2}` 激发能，Total Energy为电子态能量。
...
同样对 :math:`T_{3}` 优化可得 :math:`T_{3}` 态能量。结果显示 :math:`T_{3}` 电子态能量小于 :math:`T_{2}` 态能量，这表示 :math:`T_{2}` 与 :math:`T_{3}` 态在远离Frank-Condon区优化的过程中可能存在势能面交叉，使得最终优化的 :math:`T_{3}` 极小点能量小于 :math:`T_{2}` 。
...
自旋轨道耦合（SOC）体现的是电子自旋与电子绕核转动的相互作用。计算单重态和三重态之间的跃迁时，如果未考虑自旋轨道耦合（也就是二者耦合严格为0），那么他们的跃迁就是禁阻的；而把旋轨耦合引入哈密顿量之后，二者耦合不严格为0，此时单重态与三重态之间的跃迁就有可能发生。我们往往关心在某个特定结构下Si态与Tj态之间的自旋轨道耦合。其中<Si|SOC|Tj>表示自旋轨道耦合矩阵元，取它的模来衡量Si、Tj电子态之间的旋轨耦合作用大小。这一物理量还可以用于计算系间窜越（ISC）速率和反系间窜越（RISC）速率。

本例用BDF计算S1-T2和S1-T3态之间的旋轨耦合矩阵元，采用M062x泛函，Def2SVP基组，inp文件如下：
