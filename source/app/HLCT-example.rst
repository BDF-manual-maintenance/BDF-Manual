...
马於光课题组提出的杂化局域-电荷转移激发态（HLCT），是一种可以利用三重态激子的发光机理。通过构筑具有HLCT特征的S :sub:`1`激发态，可以增加T :sub:`n`-S :sub:`1`的旋轨耦合（El-Sayed 规则），以促进T :sub:`n`-S :sub:`1`的反系间窜越（RISC）；除此之外，增加T :sub:`n`-T :sub:`1`的能级差并减小T :sub:`n`-S :sub:`1`的能级差，也有利于反系间窜越。把给体和芳香环相连是一种构筑HLCT分子的常用手段。
...
在对分子做精确计算前我们要得到可靠的基态结构（S :sub:`0`），也就是对基态做结构优化和振动分析。首先使用Gaussian对分子基态S0进行结构优化，采用泛函B3LYP、基组6-31g** ，并考虑色散矫正。选用B3LYP泛函的原因是它计算效率高、对积分格点精度的依赖性低，并加上DFT-D3(BJ)色散校正以描述可能的非共价键作用。另外，由于对于结构优化和振动分析，其计算结果对基组的敏感性较小，选用小基组可以节约时间。gjf输入文件如下：
...
大多数有机分子的基态是闭壳层，通常是单重态，一般用S :sub:`0`表示。根据光化学第一定律（Stark-Einstein），分子需要吸收一个光子来使单电子从占据轨道跃迁到未占轨道，该光子的能量须与基态和激发态之间的能量差值一致。
...
作业完成后根据激发能绘制低激发态能级图。可以看到S :sub:`1`态和T :sub:`2`、T :sub:`3`态之间的能级差较小，若旋轨耦合矩阵元较大则存在发生系间窜越和反系间窜越的可能。
...
.. math::
    ...
    E :sub:`j`和E :sub:`i`分别为两个态的能量。基态与某个激发态之间的振子强度越大，就越容易吸收相应频率的电磁波而跃迁到那个激发态上，那么在吸收光谱中相应的吸收峰也越强。一般情况下，振子强度小于0.001可认为是跃迁禁阻。
