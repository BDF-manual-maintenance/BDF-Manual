...
.. note::
    ...
    很多程序计算的是CD，而在BDF中计算ECD，二者近似满足换算关系（参见文献 :cite:`ecd2020` 的Table S2、S3），
    换算因子可以吸收到 :math:`\alpha` 中。

为了准确计算ECD及其相对变化量，需要考虑以下两个因素：
...
计算ECD需要非常陡峭（也就是高斯指数非常大）的 *s* 型高斯基函数才能准确描述电子在铁原子核附近的分布；
...
并把 *s* 函数做非收缩处理，也就是删除 *s* 函数的收缩因子部分，并把收缩度从6改为0，然后存为其它文件名（如ANO-R2-ECD）。
由于铁没有 4*p* 价电子， *p* 函数不需要修改。
把ANO-R2-ECD放到执行计算的目录下，供后面的计算调用（下载链接 :download:`ano-r2-ecd.zip <files/ano-r2-ecd.zip>` ）。
...
计算完成后，在SCF布居分析信息之后可以找到ECD结果：

.. code-block::

  Relativistic contact densities for the atoms with Za > 25
  ----------------------------------------------------------------
...
以此为例，完成其它铁化合物分子的ECD计算（输入文件下载链接 :download:`ecd-fe.zip <files/ecd-fe.zip>` ）。
ECD结果以及 :math:`\delta^{IS}` 实验值 :cite:`neese2009` 列于下表：

.. table:: 部分铁化合物的 :math:`\delta^{IS}` 和有效接触密度
    :widths: auto

    +--------------------------+------+----------------------------+---------------------------+
    | 分子                     | 2S+1 | :math:`\delta^{IS}` (mm/s) | ECD ( :math:`bohr^{-3}` ) |
    +==========================+======+============================+===========================+
...
通过以上密度泛函理论计算得到ECD为14554.25 a.u.，代入校准方程得到 :math:`\delta^{IS}` 为0.37 mm/s，
