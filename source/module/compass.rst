...
在输出文件中以其它量子化学程序的格式打印基组和赝势。通常用于不同程序之间传递分子轨道，
因为分子轨道不仅依赖于原子顺序及坐标方位，还依赖于收缩函数的形式和排序，如果有一处不同，
就会导致传递的分子轨道无法使用。尤其需要注意的是，
即便是同名的基组，由于原始数据的来源不同以及版本的差异，
收缩函数的先后顺序以及每个收缩函数的收缩形式在不同程序中都有可能不一样。
如果在其它程序中使用 ``ExpBas`` 输出的基组，可以避免这一问题。
