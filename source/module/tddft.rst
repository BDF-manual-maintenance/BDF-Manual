...
指定TDDFT交换相关核。0为full non-collinear kernel；1为non-collinear ALDA kernel；2为no-collinear ALDA0 kernel；3为full non-collinear，产生自旋平均密度；4为full collinear kernel。对于isf=0的计算，ialda关键词不起作用；对于isf不等于0的单点计算，ialda建议设为2，因为ialda=2的结果的数值稳定性较默认的0更好；对于isf不等于0的TDDFT结构优化、TDDFT数值频率和NAC-TDDFT计算，ialda必须设为4（注意这会引入近似，导致计算结果和ialda不等于4时的结果不可比，且精度较ialda不等于4时低。也就是说，isf不等于0时的TDDFT结构优化、TDDFT数值频率的计算结果是不能与TDDFT单点能结果互相比较的）。
...
对于下述情况之一，建议用idiag=3：
 * X射线吸收/发射光谱等涉及很高的激发态的计算（详见iwindow关键字的相关说明）；
 * 计算某个能量或波长范围内的所有激发态，并且要求既不多算该范围外的激发态，又不少算该范围内的激发态（详见iwindow关键字的相关说明）。
对于下述情况，建议用idiag=2：
...
控制TDDFT初始猜测波函数。X=0: 对角元猜测；X=1: 从文件读入初始波函数（该选项仅当idiag=3时有效）；X=2: 紧束缚近似猜测；Y=0: 不存储Davidson迭代中间过程向量；Y=1: 存储Davidson迭代中间过程向量（该选项仅当idiag=3时有效）。
...
Iwindow的下一行应当包含两个浮点数，表示能量/波长范围，此外后面还可以加一个单位（au/eV/nm/cm-1），当没有给定单位时，默认单位为eV。例如以下输入表示计算激发能在1~5 eV之间的所有激发态：
...
注意对于不同的idiag值，iwindow的行为有一定区别。以上述输入为例，当使用Davidson方法时，程序既不能严格保证计算出来的所有激发态都在1~5 eV内，也不能严格保证所有在1~5 eV内的激发态都会被计算出来；不仅如此，程序必须从第一激发态开始，由低到高计算激发态，所以程序没有采取任何措施避免浪费时间计算0~1 eV内的激发态。因此，如果（1）用户需要确保该范围内的激发态不会有任何遗漏，（2）用户需要确保不会计算该范围之外的激发态，或（3）用户设定的范围的最低激发能特别高（或最长波长特别短）时，idiag=1不适用，此时应当设置idiag=3使用iVI方法。此外，iwindow不支持和idiag=2同时使用。

当指定iwindow时，iroot、nroot一般会被忽略。例外情况是对于iVI计算，极少数情况下程序报错"too small iroot/nroot, require xxx, but only yyy provided"，此时代表程序在计算前低估了最终计算出的激发态数目，因此在计算前预先分配的内存不足。这种情况下用户应当用iroot或nroot将当前不可约表示下的激发态数目设为大于等于xxx的正整数，重新进行计算。
...
------------------------------------------------
指定将波函数存储与编号为istore的文件中，以备其他计算使用。
...
---------------------------------------------------
指定打印绝对值大于cdthrd的组态激发信息。

**TD-DFT/SOC和性质计算控制参数**

:guilabel:`Nfiles` 参数类型：整型
------------------------------------------------
读入nfiles个先前的TDDFT计算的波函数，以进行SOC计算。
...
.. code-block:: bdf
     ...
     $tddft
     ....
     imatsoc         #如果imatsoc<0，所有的SOC矩阵元都会打印；如果imatsoc=0，不打印任何SOC矩阵元；如果imatsoc>0，打印imatsoc个矩阵元
...
指定在TDDFT-SOC计算里，计算标量态之间的跃迁偶极矩，imatrsf=-1可以打印所有的标量态间的跃迁偶极矩。
...
.. code-block:: bdf
     ...
     ...
     Imatrso
     5          #指定需要打印5组态之间的跃迁偶极矩，后面5行指定打印哪些态之间的跃迁矩.如果这里指定-1，则后面无需写任何信息，程序自动打印每一个态和其他态之间的跃迁偶极矩
     1 1
     1 2
     1 3
     2 3
