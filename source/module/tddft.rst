...
注意对于不同的idiag值，iwindow的行为有一定区别。以上述输入为例，当使用Davidson方法时，程序既不能严格保证计算出来的所有激发态都在1~5 eV内，也不能严格保证所有在1~5 eV内的激发态都会被计算出来；不仅如此，程序必须从第一激发态开始，由低到高计算激发态，所以程序没有采取任何措施避免浪费时间计算0~1 eV内的激发态。因此，如果（1）用户需要确保该范围内的激发态不会有任何遗漏，（2）用户需要确保不会计算该范围之外的激发态，或（3）用户指定范围的最低激发能特别高（或最长波长特别短）时，idiag=1不适用，此时应当设置idiag=3使用iVI方法。此外，iwindow不支持和idiag=2同时使用。
