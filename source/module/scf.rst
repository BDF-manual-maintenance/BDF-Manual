...
.. hint::
     读取的轨道文件必须和当前计算在以下方面相符：（1）原子的数目和种类必须相同；（2）原子的排列顺序必须相同；（3）计算使用的空间点群必须相同；（4）使用的基组必须相同；（5）要么两个计算均为RHF、RKS、ROHF或ROKS，要么两个计算均为UHF或UKS。除此之外的大多数方面不要求相同，例如原子坐标、电荷、自旋多重度、泛函等等均可以不同。其中如果（1）、（2）、（3）、（5）均满足，只有（4）不满足，可以用 ``expandmo`` 模块将轨道文件所用的基组投影到当前计算所用基组上之后，再读取轨道作为初猜（参见 :doc:`expandmo` ）。
