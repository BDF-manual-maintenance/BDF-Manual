...
相比之下，传统的TDDFT计算（与以上输入文件相同，区别仅在于去掉grimmestd关键字）耗时3264 s，结果为：
...
可以看到两个计算的激发能差别极小，在0.0~0.2 eV量级。表面上看，有一些态的振子强度差别很大，但这是因为激发能非常接近的态彼此之间混合的结果，如果作出光谱图（见 `高斯展宽的吸收光谱的绘制<plotspec>` ），可以发现sTDDFT和TDDFT的吸收光谱基本相同，两者的差别在DFT计算的正常误差范围内：
