...
利用TDDFT计算 :math:`H_{2}O` 分子激发能，简洁输入如下：
...
其中nroot关键字（也可写nexit）表明用户分别对每个不可约表示指定激发态的数目。因程序内部将 :math:`C_{2v}` 点群的不可约表示以A1、A2、B1、B2的顺序排列（见点群相关章节关于各个不可约表示的排序的介绍），因此以上输入表明只计算B1、B2激发态各一个。
...
轨道能分别是-7.62124 eV和1.23186 eV。由于 :math:`H_{2}O` 分子有4个不可约表示，TDDFT会对每个不可约表示逐一求解。
...
开壳层体系计算：U-TDDFT
----------------------------------------------------------
开壳层体系可以用U-TDDFT计算，例如对于 :math:`H_{2}O^{+}` 离子，先进行UKS计算，然后利用U-TDDFT计算激发态，一个典型的输入为，
...
常被高估。SA-TDDFT可以用于解决这一问题，考虑 :math:`N_{2}^{+}` 分子，SA-TDDFT的计算输入为,
...
BDF不仅支持TDDFT单点能（即给定分子结构下的激发能）的计算，还支持激发态的结构优化、数值频率等计算。为此需要在 ``$tddft`` 模块之后添加 ``$resp`` 模块用于计算TDDFT能量的梯度，并在 ``$compass`` 模块后添加 ``$bdfopt`` 模块，利用TDDFT梯度信息进行结构优化和频率计算（详见 :ref:`结构优化和频率计算<Optimization>` ）。
...
计算这些态的自旋轨道耦合，从下面的 :math:`CH_{2}S` 分子的sf-X2C-TDDFT-SOC计算的高级输入可以清楚地看出。
...
（1）基态和激发态之间的NACME： :math:`NO_{3}` 自由基的D0/D1 NACME（GB3LYP/cc-pVDZ）
