...
利用TDDFT计算 :math:`\rm H_{2}O` 分子激发能，简洁输入如下：
...
轨道能分别是-7.62124 eV和1.23186 eV。由于 :math:`\rm H_{2}O` 分子有4个不可约表示，TDDFT会对每个不可约表示逐一求解。
...
开壳层体系计算：U-TDDFT
----------------------------------------------------------
开壳层体系可以用U-TDDFT计算，例如对于 :math:`\rm H_{2}O^{+}` 离子，先进行UKS计算，然后利用U-TDDFT计算激发态，一个典型的输入为，
...
开壳层体系：X-TDDFT
----------------------------------------------------------
X-TDDFT是一种自旋匹配TDDFT方法，用于计算开壳层体系，开壳层体系的三重态耦合的双占据到虚轨道激发态(在BDF中标记为CV(1))存在自旋污染问题，因而其激发能常被高估。X-TDDFT可以用于解决这一问题，考虑 :math:`\rm N_{2}^{+}` 分子，X-TDDFT的计算输入为,
...
从 :math:`\rm H_{2}O` 分子闭壳层的基态出发，可以通过自旋翻转的TDDFT (spin-flip TDDFT -- SF-TDDFT)计算三重激发态，简洁输入为：
...
TDDFT **默认只计算与参考态自旋相同的激发态**， 例如，:math:`\rm H_2O` 分子的基态是单重态，TDDFT值计算单重激发态，如果要同时计算单重态与三重态，输入为：
...
计算这些态的自旋轨道耦合，从下面的 :math:`\rm CH_{2}S` 分子的sf-X2C-TDDFT-SOC计算的高级输入可以清楚地看出。
...
（1）基态和激发态之间的NACME： :math:`\rm NO_{3}` 自由基的D0/D1 NACME（GB3LYP/cc-pVDZ）
