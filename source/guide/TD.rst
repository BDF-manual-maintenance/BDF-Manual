...
基于ECP-TDDFT-SOC的自旋轨道耦合计算
----------------------------------------------------------

除了sf-X2C全电子标量相对论哈密顿以外，也可以用赝势做TDDFT-SOC自旋轨道耦合计算，其中推荐用旋轨耦合赝势（SOECP），
为此需要选择合适的 :ref:`旋轨耦合赝势基组 <soecp-bas>` ，并在 ``xuanyuan`` 模块中设置 ``hsoc`` 为10（若写其它值，都会当作10处理）。
...
.. table:: InBr分子的垂直激发能：SOECP-TDA与EOM-CCSD/SOC。能量单位：cm :math:`^{-1}`
    :widths: auto
    :class: longtable
...
除了SOECP基组以外，也可以用标量ECP基组结合有效电荷近似完成以上计算。例如，删除Br基组中的SOECP部分后，会发现结果较差：
:math:`^3\Pi_2` 与 :math:`^3\Pi_1` 的分裂只有850 cm :math:`^{-1}` ，而 :math:`^3\Sigma^+` 态的分裂几乎为零。
这是因为Br具有10个芯电子的ECP基组没有专门优化的有效电荷，程序采用实际的核电荷：
...
对于上例中的Br，不妨改用具有28个芯电子的ECP基组cc-pVTZ-ccECP：
...
这表明Br的单电子旋轨耦合积分的计算中用优化好的1435.000替换默认的核电荷数35，而对In原子仍旧计算SOECP积分。计算结果如下：

.. table:: InBr分子的TDA-SOC垂直激发能：In:SOECP，Br:SOECP与Br:ECP。能量单位：cm :math:`^{-1}`
    :widths: auto
    :class: longtable

    +---------------------+-------------+-----+-------------+-------------+-------------+-------------+
    |  Λ-S态              |   TDA       | Ω态 |   Br:SOECP  |      分裂   |     Br:ECP  |      分裂   |
    +=====================+=============+=====+=============+=============+=============+=============+
    | :math:`X^1\Sigma^+` |        0    | 0+  |         0   |             |         0   |             |
    +---------------------+-------------+-----+-------------+-------------+-------------+-------------+
...
最后，TDDFT-SOC计算也可以用SOECP（或标量ECP）基组与全电子非收缩基组的组合。例如，In继续用cc-pVTZ-PP，
而Br用全电子非相对论基组cc-pVTZ。计算得到的结果与SOECP/TDA-SOC相近，略。

.. note::

    用有效核电荷方法进行TDDFT-SOC计算的注意事项：

    #. 必须用优化好的有效电荷才能保证精度，尤其是对于ECP基组。
    #. 全电子基组必须是非相对论收缩基组，计算中完全忽略标量相对论相应，因此对于重元素不建议用全电子基组。
