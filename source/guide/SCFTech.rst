...
与其它量子化学程序传递分子轨道
------------------------------------------------
不同量子化学程序计算的分子轨道文件在原则上可以相互转化。BDF的 SCF 模块支持读入 scforb 文件和存储 scforb、molden 文件，可以通过
MOKIT（https://gitlab.com/jxzou/mokit）程序实现与其它量子化学程序之间的文件转化。

转化后的分子轨道文件正确与否，不仅依赖于原子顺序，坐标方位，以及点群对称性，还依赖于收缩函数的形式和排序，对于赝势基组还要看赝势数据是否一致。
如果有一处不一致，就会导致传递的分子轨道出现问题，无法达到加速收敛的目的。在基组方面，即便是同名的基组，由于原始数据的来源不同以及基组版本的差异，
计算的分子轨道也可能不一样。需要特别注意以下几点：

* 收缩函数的先后顺序在两个程序中是否一致？以下面H原子的cc-pVTZ基组为例，如果把s函数的三列收缩因子互换，分子轨道因子的排序是不一样的。
* 每个收缩函数的收缩形式在两个程序中是否一样？仍然以下面H原子的cc-pVTZ基组为例，第一列收缩因子仅包含前三个s原函数，即(3s)/[1s]，而在很多程序中是(4s)/[1s]，那么对应的轨道因子就会有一些差别。
* 收缩因子相位在两个程序中是否相同？这通常出现在非收缩的函数的因子 1.0 中（见第二、三列），个别程序会把收缩因子 1.0 写为 -1.0，可能导致轨道因子差个负号。
* 赝势的数据在两个程序中完全一样吗？其中最典型的是 :ref:`Def2系列基组的问题<def2-problem>` 。

为了保持基组的一致性，建议在 Compass 模块中用 ``ExpBas`` 输出基组，供其它程序使用。
...
注意damp可以和vshift联用，两者的效果在一定程度上是相互促进的。如果阻尼设为0.7仍然观察到明显的振荡，则在保证阻尼小于1的情况下增大阻尼，例如接下来可以尝试0.9、0.95等。与vshift类似，damp也是倾向于改善SCF收敛的单调性，但damp太大会导致收敛变慢，因此可以增加maxiter。当damp增加到0.99仍然无法收敛时，应该考虑其他方法。
 ...
 4. 关闭SMH，适用于第1类和第2类情况，且前3种方式都不奏效时，方法是在$scf模块里添加NoSMH关键词，增加maxiter，并视收敛情况设定vshift和damp。目前看来，起码对于基态计算而言，用SMH不收敛、不用SMH反倒能收敛的情形极其少，但是因为SMH是一种很新的SCF收敛方法，不排除极个别情况下SMH会对收敛有负面影响，因此关闭SMH可以作为一种备选方案。
