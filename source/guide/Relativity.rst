...
此外，某些全电子基组存在接近冗余的高斯指数（例如 Tb 元素的 x2c-QZVPPall-2c 基组），可能会导致数值不稳定和程序终止。
sf-X2C-AU 针对这种基组会自动调用四精度算法。
...
1. 单独计算双电子SO积分，其中可以引入一些近似降低计算量。它可以与sf-X2C标量相对论哈密顿结合，对于轻元素体系也可以与非相对论哈密顿结合。
