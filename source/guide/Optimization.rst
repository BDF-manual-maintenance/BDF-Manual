...
与原始的拉橡皮筋（Nudged Elastic Band；NEB）方法不同，CI-NEB方法在能量最高点增加了像点爬升（Climbing Image；CI）处理步骤，因此不仅能得到更准确的最低能量（反应）路径，同时还能得到过渡态结构。
...
如果只关心过渡态，也可以尝试用更少的中间像点，例如1，但对本例显然不可行，因为反应物和产物是线型结构，对中间像点的弯曲结构影响太大，很容易导致优化不收敛。
...
在能量数据之前，可以找到过渡态的直角坐标（原子单位），而在能量数据之后是其余5个像点的直角坐标（原子单位）。
这些坐标也可以从CI-NEB保存的数据文件nebpath.xyz和neb_0006.xyz中找到，其中5个像点坐标位于前者，过渡态坐标位于后者的末尾，长度单位：埃。
...
* 提取能量最高的两个像点的结构（例如本算例的3、4）作为初始结构，重新做CI-NEB优化，但是此时 ``Nebmode`` 要改为1或2，因为它们已不再是反应物和产物，做能量最小化没有意义。
