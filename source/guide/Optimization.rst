...
如果只关心过渡态，也可以用更少的中间像点，例如1，但对本例显然不可行，因为反应物和产物是线型结构，无法通过内插为过渡态产生合理的弯曲初始结构。

在本例中，指定的3个中间像点加上必须要算的3个像点，总像点数为6。其中，1、5像点对应两个端点（即反应物和产物），2、3、4为反应路径上的中间像点。
...
**Dist** 列、 **Angle** 列、、 **1-3 Ang** 列、 **1-2 Sum** 列给出了路径的特性，分别当前像点为到下一个像点的距离（可以看到NEB的像点不是等间隔的），
...
* 取结构优化最后一步的两个端点结构，以及部分或全部中间像点结构（来自数据文件nebpath.xyz），作为初始结构，重新做CI-NEB优化。输入文件如下。
  在本例中，两个端点的结构已经接近收敛，为了提高计算速度，结构保持冻结（ ``Nebmode`` =2）。
...
.. code-block:: bdf
    ...
    geometry
    C       0.0094403     -0.0045178     -0.0000013
    N       0.0051489     -1.1595576      0.0000006
    H       0.0054108      1.0740754      0.0000007
    end geometry
    nosymm
    $end
    ...
    geometry2
    # geom of image 2
    C       0.2822075     -0.0905533      0.0000240
    N      -0.4636856     -0.9729686     -0.0000113
    H       0.2014782      0.9735219     -0.0000127
    # geom of image 3
    C       0.5703159     -0.2134900     -0.0000072
    N      -0.5233808     -0.6609221      0.0000193
    H      -0.0269352      0.7844121     -0.0000121
    # geom of image 4
    C       0.7857794     -0.5068651      0.0000113
    N      -0.3940425     -0.3160241      0.0000047
    H      -0.3717369      0.7328892     -0.0000160
    # geom of image 5 (endpoint 2)
    C       0.5798873     -0.9890158      0.0000104
    N      -0.0251869      0.0162146      0.0000003
    H      -0.5347005      0.8828012     -0.0000107
    end geometry2
    $end
...
重新画像点的能量变化，如下图所示。由于中间像点使用了较好的初始结构，在优化前几步的能量已经合理了。
...
初始结构符合 :math:`\rm D_{3h}` 对称性，但因为预期优化收敛的结构可能存在虚频，在 ``compass`` 模块里用 ``nosym`` 关键字指定在 :math:`\rm C_{1}` 群下计算。程序首先收敛到 :math:`\rm D_{3h}` 点群下的极小值点：
...
.. note::
    ...
    （2）如果分子具有点群对称性，但计算时没有指定 ``nosym`` ，则可能无法完全消除所有的虚频，甚至可能导致结构优化不收敛。例如在以上算例中，如果不指定 ``nosym`` ，即令计算在分子的实际点群（ :math:`\rm D_{3h}` ）下进行，则因为不管如何消除两个虚频都会破坏 :math:`\rm D_{3h}` 对称性，使得无法消除虚频，进而导致优化不收敛。
