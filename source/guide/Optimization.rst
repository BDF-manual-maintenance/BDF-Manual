...
优化CI需要调用DL-FIND外部库 :cite:`dlfind2009` ，为此需要在BDFOPT模块的输入里设定 ``solver`` 为 0，或者不写。
相应地，设定 ``solver`` 为 1 代表使用BDF自带的结构优化代码而非DL-FIND来进行优化。原则上，极小值点和过渡态的优化也可用DL-FIND来实现，但效率一般不如BDF自带代码好，因此仅对于BDF自身代码不支持的CI优化等任务，才应调用DL-FIND。
