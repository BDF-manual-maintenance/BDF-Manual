...
基态结构优化：一氯甲烷（ :math:`CH_{3}Cl` ）在B3LYP/def2-SV(P)水平下的结构优化
---------------------------------------------------------------------------------
...
以上述 :math:`CH_{3}Cl` 的结构优化任务为例，可以看到 ``.out`` 文件里BDFOPT模块的输出：
...
频率计算：:math:`CH_{3}Cl` 在平衡结构下的谐振频率及热力学量的计算
-------------------------------------------------------------------------
...
计算的输出与优化极小值点结构类似。最后频率分析时可以看到收敛的结构有且仅有一个虚频（-1104 :math:`cm^{-1}`）：
 ...
 * 在COMPASS模块中指定采用分子点群的某一个子群，此时程序只会保持该子群对称性不被破坏。若指定的是 :math:`C_1` 群，则程序允许以任何方式破坏分子对称性，可以最大程度上提高得到低能量结构的概率，但代价为无法利用点群对称性加速计算，导致计算量增加。
