...
CI-NEB计算需要提供两个端点的坐标，其中第一个端点（这里取反应物）的初始结构在 ``Compass`` 模块提供，是加了一点扰动的线型HCN。
...
由于CI-NEB方法的中间像点数越多计算越慢，且增加结构不收敛的几率，因此不建议用太多的中间像点。本例用了较少的中间像点，并用 ``Crude`` 降低收敛精度，同时对反应物和产物做能量最小化（默认固定不优化）。
如果只关心过渡态，也可以用更少的中间像点，例如1，但对本例显然不可行，因为反应物和产物是线型结构，无法通过内插为过渡态产生合理的弯曲初始结构。

本例中指定了3个中间像点，加上必须要算的3个像点，总像点数为6，其中，1、5像点对应两个端点（即反应物和产物），2、3、4为中间像点，像点6为过渡态。
...
输出文件往前翻，可以看到各个像点（包括反应物、产物、过渡态）的能量：
...
可以看到，CI-NEB方法得到的过渡态总能量是-93.2241949 Hartree，与Dimer方法优化得到的过渡态总能量-93.22419648 Hartree非常接近。

在能量数据之前，可以找到过渡态的直角坐标（原子单位），而在能量数据之后是最低能量路径上5个像点的直角坐标（原子单位）。
这些坐标也可以从CI-NEB保存的数据文件nebpath.xyz和neb_0006.xyz中得到，其中5个像点坐标位于前者，过渡态坐标位于后者的末尾，长度单位：埃。
...
或者收敛到我们不想要的激发态上。遇到不收敛的CI-NEB计算，可以提取能量两个像点的结构（例如本算例的3、4）作为初始结构，
重新做CI-NEB优化，但是此时 ``nebmode`` 要改为1或2，因为它们已不再是反应物和产物，做能量最小化没有意义。
