...
全电子相对论计算要用到DKH、ZORA、X2C等考虑相对论效应的哈密顿（见 :doc:`相对论效应<relativity>` ），
...
如果是高级输入模式，计算采用的基组在 ``compass`` 模块中指定，利用关键词 ``basis`` 。例如
...
上例中， ``H2`` 类型的两个氢原子用cc-pVDZ基组， ``H2`` 氢原子用3-21G基组，碳原子用6-31G基组。需要注意的是，对称等价原子必须使用相同基组，程序将对此进行检查；
